        ...
        stage('ScanCodeBase')
        {
            ...
            steps
            {
                withCredentials([
                    ...
                    {
                        sh('''#!/usr/bin/env bash
                        sudo docker run --mount type=bind,src=/home/achilles/Documents/pubmed_scrape,dst=/repo cr.sigsci.dev/whitesource:latest -c /repo/config.w.key -d /repo -product pubmed_scrape_product -project pubmed_scrape_project
