        ...
        stage('Build')
        {
            steps
            {
                    ...
                    ])
                {
                    ...
                    docker run --mount type=bind,src=.,dst=/repo --env CLI_OPTIONS="--userKey $USERAPIKEY --apiKey $ORGAPIKEY -c /repo/wss-unified-agent.config -d /repo -project goof" nexus.corp.signaturescience.com/repository/sigsci-docker-registry/whitesource-agent:latest 
                    ''')
                }
            }
        }
